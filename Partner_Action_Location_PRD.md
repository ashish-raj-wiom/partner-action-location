...
| **Version** — v0.6 · 30 Jul 2026 | **Consulted — Legal/DPDP** — TBD | **Consulted — Android** — TBD | **Consulted — Analytics** — TBD |

---

## 1. Objective & Definition of Success

**Objective.** For any action a partner user takes in the CSP app or the Technician app, Wiom can tell where that person physically was — as accurately as the device can report it, with every reading declaring its own reliability.
...
| R2 | As an analyst acting on a named individual's data, I need the most accurate reading the device can give, and I need each one to declare how far it can be trusted — so I never place a person somewhere on the strength of a city-level or hours-old reading. | **(a)** Record the most accurate location the device can provide without delaying the action, together with its accuracy radius in metres. **(b)** Record the age of the reading in seconds. **(c)** Record whether the reading came from a mock-location provider. **(d)** Capture the location at the moment the action happens and record it as part of that action, never by a later pass over actions already recorded. | Record a latitude or longitude without its accuracy, its age and its mock flag (G1). |
| R3 | As an analyst, I need to know why a coordinate is missing, so that absence is a finding rather than a hole. | **(a)** Record a location status against **every** partner action, including actions with no coordinate, drawn from: fresh reading · stale reading · no reading available ⚠️ *AI GENERATED — review* · device location switched off. | Record a partner action with the status absent or empty (G2). |
...
flowchart TD
    ...
    K -- "No" --> L["T6 — status only: no reading available, record without waiting"]
...
| T6 | Available · Device location off | Partner user performs an action | Dispatched by the §3a action chart | *No change* | Record carries: coordinate when a reading is available; accuracy in metres, age in seconds and mock flag whenever a coordinate is present (R2a, R2b, R2c, G1); location status always (R3a, G2); the user's role (R6a). The location is recorded in real time as part of the action (R2d) and the action is never held to wait for one (P1, G3). |
...
|---|---|---|---|---|
| C-01 | Maximum reading age still recorded as a fresh reading (R3a, T6) | 60 seconds ⚠️ *AI GENERATED — review — pending engineering review* | 30–300 s ⚠️ *AI GENERATED — review — pending engineering review* | Engineering |
| C-02 | Target accuracy for a recorded reading — as accurate as the device can give without delaying the action (R2a) | Pending engineering review ⚠️ *AI GENERATED — review* | Pending engineering review ⚠️ *AI GENERATED — review* | Engineering |
...
| MQ-2 | For each partner action: the accuracy radius, the age, and whether the reading was mock. | G1 · R2a · R2b · R2c · C-01 · C-02 |
...
| MQ-8 | The share of signed-in partner users whose records carry a populated role. | R6a · R6b |
...
| AC-CAP-4 | **Given** Sunil in a session where no reading has yet been obtained, **When** he taps a CTA, **Then** the record carries status *no reading available* — the status is present and non-empty. | R3a · T6 · G2 | Settled |
| AC-CAP-5 | **Given** a device running a mock-location provider reporting 28.4389, 77.0512 while Sunil is two kilometres away, **When** he submits any install step, **Then** the record carries that coordinate with mock flag **true**. | R2c · T6 · G1 · MQ-9 | Settled |
...
| AC-CAP-8 | **Given** Sunil acting at 14:00:00, **When** his action is recorded, **Then** the location was captured as part of that action at 14:00:00 — not attached to it by a later pass. | R2d · T6 | Settled |
...
| AC-SESS-7 | **Given** a MANAGER+ user, **When** they act in the CSP app and later in the Technician app, **Then** their records carry role MANAGER+ in both cases. | R6a · R6-MUST NOT | Settled |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-RACE-1 | **Given** a location request in flight with no reading yet, **When** Sunil acts and the reading arrives 200 ms later, **Then** his action was already recorded with status *no reading available*, and the late reading is not attached to it. | P1 · R2d · R3a · T6 | Settled |
| AC-RACE-2 | **Given** a reading already available and two actions performed within the same 100 ms, **When** both are recorded, **Then** both carry that same reading, each with its own age measured at its own instant. | P1 · R2b · T6 | Settled |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-GRD-1 | **Given** any sample of partner records carrying a coordinate, **When** the sample is inspected, **Then** every one also carries accuracy, age and mock flag — zero coordinates appear without all three. | G1 · R2a · R2b · R2c · R2-MUST NOT · MQ-2 | Settled |
...
| AC-GRD-5 | **Given** a week of captured records, **When** the PM queries for Ramesh Kumar's actions between 25 and 31 Jul, **Then** the result lists each action with its coordinate, accuracy, age, mock flag, status, role, app and service. | R5a · MQ-1 | Settled |
...
| Age | Seconds between the instant a reading was obtained and the instant the action happened. Distinct from accuracy: a reading can be precise and old, or fresh and vague. | — |
...
|---|---|
| Obtain the most accurate device location available without delaying the action, carrying an accuracy radius, the instant it was obtained, and a mock-provider indication. | R2a · R2b · R2c · T6 · P1 · C-02 |
| Capture the location at the instant an action happens and carry it with that action, rather than reconciling it onto the action later. | R2d · AC-CAP-8 · AC-RACE-1 |
...
| Change C-01 without an app release. | C-01 · AC-CFG-1 |
| Distinguish "no reading available" from "device location switched off" as separate observable states. | R3a · G2 · MQ-3 |
| Record the role of the signed-in partner user, independent of which app they signed in from, and re-establish it when the signed-in user changes without an app restart. | R6a · R6b · MQ-8 · T7 |
...
|---|---|---|---|
| 1 | §5 C-01 (60 s, 30–300 s) | Fresh/stale boundary. **Owner set to Engineering pending their review.** | Common practice for treating a cached reading as current. |
| 2 | §5 C-02 | The parameter itself — a target accuracy, value and range left to Engineering. | Created to carry your "as accurate as possible without hampering latency" requirement as something engineering can set and change. |
| 3 | §2 R3a *no reading available* | A fourth status value for "permission and device setting both fine, but no reading obtained yet". | Without it, those actions get labelled *device location switched off*, which is a different fact. |
| 4 | §3b T7 · §2 R6b · AC-SESS-5 · §9 | **Shared-handset user switch**, including that no reading crosses a user boundary. | Partner handsets are shared between an OWNER and technicians; without this, one person's location can be attributed to another. |
| 5 | §3b T8 | No reading retained by the app between sessions. | Data-minimisation default; not discussed. |
| 6 | §2 R6 · MQ-8 · AC-SESS-7 | **The whole role-capture requirement.** | Your objective is OWNER vs TECHNICIAN vs MANAGER vs MANAGER+. Role is not reliably available in analysis today, and the backend transition log flattens every partner role to `'CSP'`. Without role, the by-role question is unanswerable. |
| 7 | §1 G1–G3 names and wording | Three of the four guardrails. | Derived from your decisions; the naming and the split are mine. G4 is confirmed. |
| 8 | §6 MQ-9 | Mock-location reporting. | Follows from R2c; you approved capturing the flag but not what it feeds. |
...
| **OV-5** — §5 requires a default and a range for every C-id. | C-02 carries neither; both are delegated to Engineering. | The product requirement is "as accurate as the device can give without delaying the action". Any number the PM invented would be a guess about device and latency behaviour that Engineering is better placed to set. | Ashish Raj (PM), 30 Jul 2026 |
