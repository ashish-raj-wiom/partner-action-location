...
| **Version** — v0.7 · 30 Jul 2026 | **Consulted — Legal/DPDP** — TBD | **Consulted — Android** — TBD | **Consulted — Analytics** — TBD |

---

## 1. Objective & Definition of Success

**Objective.** For any action a partner user takes in the CSP app or the Technician app, Wiom can tell where that person physically was — as accurately as the device can report it, and with a fabricated reading never passing as a measured one.

> **No customer outcome.** This is an internal capability; no customer experience changes. Recorded as **OV-1**.

**Boundary.** This spec governs **what must be recorded about where a partner user was when they acted**, in both apps, across every module, service and role (C-02). It changes no service's behaviour, no state machine and no task outcome. The **customer app is entirely out of scope** (AC-REG-1). Obtaining a reading must never make an action slower or less reliable (R4, G3). The existing use of location for WiFi scanning during router configuration must continue to work unchanged (AC-REG-2). This spec captures where each action happened; it defines no comparison against any reference point.
...
|---|---|---|---|
| G1 | **Self-describing readings** ⚠️ *AI GENERATED — review* | No coordinate is ever recorded without its accuracy radius and its mock flag. | R2a · R2b · AC-CAP-1 · AC-GRD-1 · MQ-2 |
...
flowchart TD
    ...
    K -- "Yes" --> M["T6 — coordinate plus accuracy and mock flag, status location recorded"]
    ...
    O -- "Signed-in user replaced" --> R["T7 — re-evaluate as a new session"]
...
| T6 | Available · Device location off | Partner user performs an action | Dispatched by the §3a action chart | *No change* | Record carries: coordinate when a reading is available; the accuracy radius in metres and the mock flag whenever a coordinate is present (R2a, R2b, G1); location status always (R3a, G2); the user's role (R6a). The location is fetched when the action happens (R2c) and the action is never held to wait for one (P1, G3). |
...
|---|---|---|---|---|
| C-01 | Target accuracy for a recorded reading — as accurate as the device can give without delaying the action (R2a) | Pending engineering review ⚠️ *AI GENERATED — review* | Pending engineering review ⚠️ *AI GENERATED — review* | Engineering |
...
|---|---|---|---|
| AC-CAP-1 | **Given** Sunil signed in and the device able to report 28.4390, 77.0511 with a 12 m accuracy radius, **When** he proposes a slot on BKG-2026-07-118342, **Then** the action's record carries that latitude and longitude, accuracy 12 m, mock flag false, status *location recorded*, and role TECHNICIAN. | R1a · R2a · R2b · R3a · R6a · T6 · G1 | Settled |
| AC-CAP-2 | **Given** the same conditions but the best the device can report is a 2,400 m accuracy radius, **When** he acts, **Then** the record carries that coordinate with accuracy 2,400 m and mock flag false — the coordinate is still recorded, and the action is not delayed to obtain a better one. | R2a · T6 · P1 | Settled |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-SESS-1 | **Given** Sunil opening the Technician app with the device location setting on, **When** the session starts, **Then** his role is recorded as TECHNICIAN and his first action carries a coordinate with its accuracy radius and mock flag. | T1 · T2 · R6a · R2 · G1 | Settled |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-WF-1 | **Given** Sunil with the device location setting on, **When** he completes the on-site sequence for BKG-2026-07-118342 from arrival through to the customer rating, **Then** every step in that sequence has a record carrying a coordinate, its accuracy radius, its mock flag, a status and a role — with no gaps in the sequence. | R1a · R1b · R2 · R6a · T6 · G1 | Settled |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-DUP-1 | **Given** Sunil double-tapping a CTA 400 ms apart with a reading available, **When** both actions are recorded, **Then** each carries the coordinate, its accuracy radius and its mock flag — neither record is dropped, and the two are not merged into one. | R2a · R2b · T6 · P1 | Settled |
...
| AC-GRD-5 | **Given** a week of captured records, **When** the PM queries for Ramesh Kumar's actions between 25 and 31 Jul, **Then** the result lists each action with its coordinate, accuracy radius, mock flag, status, role, app and service. | R5a · MQ-1 | Settled |
...
|---|---|
| Obtain the most accurate device location available without delaying the action, carrying an accuracy radius and a mock-provider indication. | R2a · R2b · T6 · P1 · C-01 |
...
|---|---|---|---|
| 1 | §5 C-01 | The parameter itself — a target accuracy, value and range left to Engineering. | Created to carry your "as accurate as possible without hampering latency" requirement as something engineering can set and change. |
| 2 | §2 R3a | **The three status values** — *location recorded* · *no reading available* · *device location switched off*. | The set has to distinguish a successful capture from the two distinct reasons a coordinate can be missing, or MQ-3 cannot separate them. |
| 3 | §3b T7 · §2 R6b · AC-SESS-5 · §9 | **Shared-handset user switch**, including that no reading crosses a user boundary. | Partner handsets are shared between an OWNER and technicians; without this, one person's location can be attributed to another. |
| 4 | §3b T8 | No reading retained by the app between sessions. | Data-minimisation default; not discussed. |
| 5 | §2 R6 · MQ-8 · AC-SESS-7 | **The whole role-capture requirement.** | Your objective is OWNER vs TECHNICIAN vs MANAGER vs MANAGER+. Role is not reliably available in analysis today, and the backend transition log flattens every partner role to `'CSP'`. Without role, the by-role question is unanswerable. |
| 6 | §1 G1–G3 names and wording | Three of the four guardrails. | Derived from your decisions; the naming and the split are mine. G4 is confirmed. |
| 7 | §6 MQ-9 | Mock-location reporting. | Follows from R2b; you approved capturing the flag but not what it feeds. |
...
| **OV-5** — §5 requires a default and a range for every C-id. | C-01 carries neither; both are delegated to Engineering. | The product requirement is "as accurate as the device can give without delaying the action". Any number the PM invented would be a guess about device and latency behaviour that Engineering is better placed to set. | Ashish Raj (PM), 30 Jul 2026 |
| **OV-6** — §7 requires boundary-value and configurability ACs where a limit or a runtime-changeable parameter exists. | Neither group appears. | No numeric threshold sits in the product's scope. C-01's value is Engineering's to set, and there is no other PM-owned limit to test at its edges. | Ashish Raj (PM), 30 Jul 2026 |
