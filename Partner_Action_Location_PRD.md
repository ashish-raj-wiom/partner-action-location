...
| R2 | As an analyst acting on a named individual's data, I need the most accurate reading the device can give, and I need each one to declare how far it can be trusted — so I never place a person somewhere on the strength of a city-level or hours-old reading. | **(a)** Record the most accurate location the device can provide without delaying the action, together with its accuracy radius in metres. **(b)** Record the age of the reading in seconds. **(c)** Record whether the reading came from a mock-location provider. **(d)** Record the location in real time, as part of the action, rather than attaching it afterwards. | Record a latitude or longitude without its accuracy, its age and its mock flag (G1). |
...
| AC-RACE-2 | **Given** two actions performed within the same 100 ms, **When** both are recorded, **Then** each carries its own reading and its own age at its own instant. | P1 · R2b · T6 | Settled |
...
| Record the location as part of the action, in real time, rather than attaching it in a later pass. | R2d · AC-CAP-8 · AC-RACE-1 |
